...
function CharAnimatedText({item, index, length}) {
    ...
    const applyTransitionOldVisor = useMemo(() => {
        return index === length - 2 && !isOperator
    }, [index])

    const isLastOperator = useMemo(() => {
        return index === length - 1 && isOperator
    }, [index])

    const y = useSharedValue(isOperator ? -95 : 0)
    const opacity = useSharedValue(isOperator ? 0 : 1)
    const fontSize = useSharedValue(applyTransitionOldVisor ? 100 : 30)
    const rotate = useSharedValue(isLastOperator ? 180 : 0)
